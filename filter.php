...
<body>
    ...
    <script>
        ...
        async function fetchCategoriesAndChannels() {
            ...
            try {
                const categoryRes = await fetch(`${baseURL}&action=get_live_categories`);
                categories = await categoryRes.json();               
                displayCategories(categories);

                const channelRes = await fetch(`${baseURL}&action=get_live_streams`);
                channels = await channelRes.json();
                showPopup("Categories and channels loaded successfully!");
            } catch (error) {
                console.error("Error fetching data:", error);
                showPopup("Failed to fetch categories or channels. Please check your IPTV details in data/data.json.");
            } finally {
        ...
        async function saveM3U() {
            ...
            try {
                ...
                showPopup("Failed to save playlist due to a network error.");
            }
        }
